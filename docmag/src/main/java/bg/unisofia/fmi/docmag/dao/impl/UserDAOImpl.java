...
public class UserDAOImpl implements UserDAO {
        ...
        public List<User> getAllUsersOfType(UserType type) {
        	Query searchUserQuery = new Query(Criteria.where("type").is(type.toString()));
		...
		public List<Teacher> getScientificLeadersForPHDStudent(PHDStudent phdStudent) {
			...
			if (leaderIds != null && leaderIds.size() > 0) {
				Query leadersQuery = new Query(Criteria.where("_id").in(phdStudent.getScientificLeaderIds()));
		...
		public List<Teacher> getScientificLeadersForThesis(ThesisProposal thesis) {
			Query leadersQuery = new Query(Criteria.where("_id").in(thesis.getScientificLeaderIds()));
			List<Teacher> leaders = mongoTemplate.find(leadersQuery, Teacher.class);
		...
		public List<User> getConsultantsForThesis(ThesisProposal thesis) {
			Query consultantsQuery = new Query(Criteria.where("_id").in(thesis.getConsultantIds()));
			List<User> consultants = mongoTemplate.find(consultantsQuery, User.class);
		...
		@Override
		public boolean assignConsultantForThesis(ThesisProposal thesis,
				User user) {
			// TODO Auto-generated method stub
			return false;
		}
